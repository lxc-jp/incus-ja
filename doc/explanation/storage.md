...
| ストレージロケーション        | Directory | Btrfs    | LVM (すべて) | ZFS      | Ceph (すべて) | LINSTOR  | TrueNAS  |
| :---                          | :---      | :---     | :---         | :---     | :---          | :---     | :---     |
| ホストと共有                  | &#x2713;  | &#x2713; | -            | &#x2713; | -             | -        | -        |
| 専用のディスク/パーティション | -         | &#x2713; | &#x2713;     | &#x2713; | -             | &#x2713; | -        |
| ループディスク                | -         | &#x2713; | &#x2713;     | &#x2713; | -             | &#x2713; | -        |
...
```{note}
書き込みはその都度ストレージドライバーとメインドライブのファイルシステムを通過する必要があるため、パフォーマンスは低くなります。

さらにホストのファイルシステムがcopy-on-write（`btrfs`か`zfs`）の場合、ループファイル内の各コミットがホストのファイルシステムによって再度コミットされる必要があるため、同期的な書き込みのワークロードは、占有したディスクやパーティションへの物理的な書き込みにくらべて数倍になることもあります。SSDではスループットだけではなく寿命にも影響します。
```
...
クラスターでは、ローカルストレージのストレージバケットは単一のクラスターメンバーに所属し、データはレプリケーションされません。
