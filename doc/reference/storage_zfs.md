...
Incusは、ストレージプールの作成時に与えたもの（ZFSプールまたは{spellexception}`dataset`）に対して完全な制御を持っていると想定します。
このため、子供の{spellexception}`datasets`やファイルシステムエンティティーをその中に決して作らないでください。Incus が消してしまうかもしれないからです。
