...
プロキシデバイスを`nat=true`に設定する際は、ターゲットのインスタンスのNICに静的IPを設定するか動的な（DHCP）アドレスに依存することができます。
静的なアドレスを設定しない場合、ワイルドカードの接続アドレス（IPv4では`0.0.0.0`、IPv6では`[::]`）を使ってブリッジのネイバーテーブルからインスタンスの現在のアドレスを自動検出し、アドレスが変わるたびにNATルールが更新されます。
...
リッスンアドレスもワイルドカードのアドレス（IPv4では`0.0.0.0`、IPv6では`[::]`）をNATと非NATモードの両方で使えます。
NATモードではワイルドカードのリッスンアドレスはIncusホストで使われる宛先アドレスに関係なくトラフィックをインスタンスにフォワードします。
