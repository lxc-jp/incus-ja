...
`bridged`、`p2p`、`routed`インターフェースタイプは`queue.discipline`オプションを指定でき、NICのホスト側で使われるキューイング規律を選べます。

デバイスにレートリミットが設定される際、制限を実装するクラスにキューイング規律が付与されます。パケットがキューイングされる場所だからです。
レートリミットがない場合は、ホスト側のインターフェースのrootのキューイング規律になります。

仮想マシンでは、`queue.discipline.attach`でキューイング規律が付与される場所を選びます。
仮想マシンのホスト側のインターフェースはマルチキューのTAPデバイスですので、デフォルト（`queue`）ではキューイング規律はそれぞれの転送キューの`mq` rootの下に付与されます。
`root`に設定すると、代わりにインターフェースのrootに単一のキューイング規律が置かれ、すべての転送キューを直列化します。
イングレスのレートリミットが設定されている場合は`queue`に設定はできません。そのリミットはインターフェースrootを所有するからです。
