...
仮想マシンは`rootfs/`ディレクトリーの代わりに単一の`rootfs.img`ファイルを使います。このファイルは`qcow2`でフォーマットされていることを想定しています。
...
1 つは （通常`*.tar.xz`）はメタデータと省略可能なテンプレートファイルを含む tarball です。
もう 1 つはルートファイルシステム（コンテナの場合）、あるいはrootディスク（仮想マシンの場合）です。
