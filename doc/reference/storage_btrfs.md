...
ボリューム毎の`btrfs.compression`オプションはこのクォータの問題に役立ちます。というのは、圧縮を有効にすることにより最大のエクステントサイズが縮小し、ブロックをリライトしてもストレージが2倍消費されにくくなるからです。
しかし、この2つは相容れないため、Incusがボリュームのコピー・オン・ライトを無効にしなくなります。
コピー・オン・ライトを無効なままにするには`btrfs.compression=none`を設定してください。
```
