...
インスタンスは`security.tags`設定キーにカンマ区切りのタグを設定することでタグ付けできます。
Incusは自身の認可の決定にこれらのタグは使いません。
代わりに、各タグはサーバーに関連した`security_tag`オブジェクトとしてOpenFGA内で公開され、タグを保持する各インスタンスに`tag`リレーションを保持します（例えば、`security_tag:pci tag instance:default/web1`）。
これによりOpenFGAストアーに直接アクセスする外部ツールにタグを見せることができます。
...
- `details`：以下のアトリビュートを持つオブジェクト
   ...
   - `Claims`: 辞書形式のバリデーションされたOIDCトークンクレーム（非OIDCクライアントでは空）
