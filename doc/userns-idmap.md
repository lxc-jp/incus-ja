...
特定のマッピングを使うには、`security.idmap.base`キーを設定するとコンテナのベースとしてホストのどのUID/GIDを使うかを、`security.idmap.size`（デフォルトは65536）で指定する範囲のサイズとともに、Incusに指定できます。この範囲は分離されず、同じベースを使う複数のコンテナで共有できます。そのため、`security.idmap.isolated`で結合はできません。分離されたコンテナはこれらの範囲とは別に範囲を割り当てられます。
