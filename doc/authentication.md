...
トークンは調整可能な時間（{config:option}`server-core:core.remote_token_expiry`）を過ぎるか一度使用すると無効になるため、これはトラストパスワードを使用するよりも安全な方法です。
...
1. すべてのマシンに{abbr}`CA（Certificate authority、認証局）`の証明書を追加します:
   ...
   - `client.ca`ファイルをクライアントの設定ディレクトリー（`~/.config/lxc`）に置く。
   - `server.ca`ファイルをサーバーの設定ディレクトリー（`/var/lib/lxd`）に置く。
...
```{note}
OpenID Connect 認証は現在開発中です。
Incus 5.13 <!-- TODO: バージョン修正 ->から、OpenID Connect を通じた認証がサポートされていますが、まだユーザーロールの取り扱いはありません。
