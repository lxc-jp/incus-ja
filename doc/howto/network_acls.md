...
| プロパティ         | 型     | 必須 | 説明                                                                                                                                                                    |
| :---               | :---   | :--- | :---                                                                                                                                                                    |
| `action`           | string | yes  | マッチしたトラフィックに適用するアクション（`allow`, `allow-stateless`, `reject` または `drop`）、`allow-stateless`はOVNネットワークのみでサポート                      |
...
- OVN ACL とは違い、ブリッジ ACL はブリッジと Incus ホストの間の境界のみに適用されます。
  これは外部へと外部からのトラフィックにネットワークポリシーを適用するために使うことしかできないことを意味します。
  ブリッジ間（{spellexception}`intra-bridge`）のファイアウォール、つまり同じブリッジに接続されたインスタンス間のトラフィックを制御するファイアウォールには使えません。ただし、ACLが直接NICデバイスに適用される場合を除きます。その場合は`nftables`の制約を回避するため、`reject`のACLルールがイングレスのトラフィックに適用される場合は`drop`に変換します。
