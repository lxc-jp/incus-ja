...
- `bgp.ipv4.instances`か`bgp.ipv6.instances`がネットワークで有効な場合は個々のインスタンスのアドレス（IPv4の`/32`とIPv6の`/128`）
...
### 個々のインスタンスアドレスを広告する（`bridge` のみ）

デフォルトでは、ブリッジネットワークは自身のサブネット（あるいはNATアドレス）のみを広告します。
（例えば今インスタンスを稼働させている特定のサーバーの共通のサブネットへルーティングするために）代わりに個々のインスタンスアドレスへのルートを広告したい場合は、ネットワーク上で`bgp.ipv4.instances`や`bgp.ipv6.instances`を有効にしてください。

有効にすると、Incusはネットワークに接続した稼働中の各インスタンスへの`/32`（IPv4）または`/128`（IPv6）ルートを広告し、インスタンス停止時には取り下げます：

- インスタンスのNICに静的な`ipv4.address`か`ipv6.address`が設定されている場合は、インスタンスが起動したらすぐにそのアドレスが広告されます。
- それ以外の場合、インスタンスの起動後少ししてからカーネルのネイバーテーブル内でNICのMACアドレスに紐づけられた動的に割り当てられた（DHCPv4、DHCPv6、SLAAC）アドレスをルックアップします。インスタンスのアドレスが見えるようにするために、インスタンスは起動後すぐに何らかのネットワークトラフィックを生成する必要があります。
...
- `bgp.peers.<name>.interface` - （ピアアドレスの代わりに）BGP unnumberedピアリングに使うインターフェース名
...
ピアアドレスを設定する代わりに、`bgp.peers.<name>.interface`を設定することでBGP unnumberedを使ってセッションを確立させることができます。
このモードでは、ピアのIPv6リンクローカルアドレスが指定のインターフェースで自動検出され、拡張されたネクストホップ機能を使ってIPv4のルートが交換されます。

これによりBGPピアリングのすべてのホストにグローバルアドレスを割り当てる必要がなくなり、大きなクラスターでは特に有用です。
利用にはアップストリームのルーターもBGP unnumberedをサポートしており、そのインターフェースに直接接続されていることが必要です。

アップリンクのネットワークが一旦設定されると、ダウンストリームのOVNネットワークはBGP上で広告される外部のサブネットとアドレスを受け取ります。
ネクストホップはアップリンクのネットワークのOVNルーターのアドレスに設定されます。
