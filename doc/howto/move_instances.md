...
クラスター間のライブマイグレーションでは、`--target`に加えて`--target-project`を指定することでインスタンスを別のプロジェクトに移動することもできます。
稼働中のインスタンスは異なるデバイスセットを設定できないため、インスタンスは異なるクラスターメンバーに移動する必要があり、デバイスは移動先のプロジェクトで同じセットになる必要があります。
アタッチされたカスタムボリュームは`dependent`とマークされたものは移動されますが、そうでないボリュームはインスタンスに追随しないので別途移動が必要です。
