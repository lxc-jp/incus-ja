...
func (s *Stmt) create(buf *file.Buffer, replace bool) error {
	...
	for _, field := range all {
		column, value, err := field.InsertColumn(mapping, table, s.defs, s.registeredSQLStmts, all)
		if err != nil {
			return err
		}
...
func (s *Stmt) rename(buf *file.Buffer) error {
	...
	for _, field := range nk {
		column, value, err := field.InsertColumn(mapping, table, s.defs, s.registeredSQLStmts, nk)
		if err != nil {
			return err
		}
...
func (s *Stmt) delete(buf *file.Buffer) error {
	...
	if strings.HasPrefix(s.kind, "delete-by") {
		...
		for _, filter := range filters {
			...
			column, value, err := field.InsertColumn(mapping, table, s.defs, s.registeredSQLStmts, nil)
			if err != nil {
				return err
			}
